...
// Add the Gift with purchase data source ID here
...
class GiftWithPurchase {
  ...
  async enrichCartData() {
    ...
    if (enrichedProducts && enrichedProducts.length > 0) {
      ...
      const variantMap = this.mapEnrichedProductVariantsWithCart(
        this.enrichedCart,
        cartItems
      );
      ...
      for (const item of Tapcart.variables.cart.items) {
        if (item?.attributes) {
          ...
          if (
            ...
            item.attributes._attribution === "Rebuy Tapcart Gift with Purchase"
          ) {
            isGiftItem = true;
          }
          if (isGiftItem) {
  ...
  mapEnrichedProductVariantsWithCart(enrichedProducts, cartProducts) {
    ...
    for (const product of cartProducts) {
      ...
      const enrichedVariant =
        enrichedProduct?.variants?.find(
          (variant) => variant.id === Number(product.variantId)
        ) || {};
  ...
  async getProducts() {
    ...
    if (cartItems?.length > 0) {
      ...
      // Turn to comma separated string
  ...
  giftWithPurchaseAutoAdjust() {
    ...
    if (giftItemsInCart.length > 0) {
      giftItemsInCart.forEach(({ variantId, quantity }) => {
        if (!giftVariantIds.includes(variantId)) {
          giftsToRemoveFromCart.push({ variantId, quantity });
        }
      });
    }
    ...
    if (giftVariantIds.length > 0) {
      giftVariantIds.forEach((id) => {
        const foundGift = giftItemsInCart.find(
    ...
    if (giftsToRemoveFromCart.length > 0) {
      this.removeItem(giftsToRemoveFromCart);
    }

    // If a customer tries to increase the item quantity
  ...
  getCartSubtotalWithoutGiftItems() {
    ...
    console.error(
      "Was not able to calculate cart subtotal",
      "Calculated = ",
      calculatedSubtotal,
      ": Tapcart Subtotal = ",
      subtotal
  ...
  addToCart(variants = []) {
    ...
    for (const variantId of variants) {
      data.lineItems.push({
        ...
        attributes: [
          ...
          {
            key: "_widget_id",
            value: widgetId, // Rebuy Tapcart Gift with Purchase
          },
        ],
      });
    }
    ...
    console.log("%cAdd to cart", "color: green", data);

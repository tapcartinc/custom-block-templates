...
const util = {
    getSelectedVariant: (product: ProductType) =>
        product!.variants.find(({ id }) => id === product?.selectedVariant.id)!,
    getSelectedOptionMap: (variant: VariantType) =>
        variant.selectedOptions.reduce<Record<string, string>>((acc, { name, value }) => {
            acc[name] = value;
            return acc;
        }, {}),
            ...
            values: optionValues.map((optionValue) => ({
                ...
                getRelevantVariant: (selectedOptionMap: Record<string, string>) => {
                    const relevantVariant =
                        ...
                        Tapcart.variables
                            .product!.variants.filter(({ selectedOptions }) =>
...
const render = {
    ...
    swatches: (productId: string, productOption: ProductOption) => {
        ...
        const optionValues = productOption.values.map<
            ProductOptionValue & { element: HTMLLabelElement }
        >((option) => {
